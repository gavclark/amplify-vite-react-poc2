...
import { getKpiDefinition } from "./graphql/queries";
import { updateKpiDefinition } from "./graphql/mutations";
const client = generateClient();
export default function KpiDefinitionUpdateForm(props) {
  const {
    id: idProp,
    kpiDefinition: kpiDefinitionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = kpiDefinitionRecord
      ? { ...initialValues, ...kpiDefinitionRecord }
      : initialValues;
    setKpiCode(cleanValues.kpiCode);
    setKpiArea(cleanValues.kpiArea);
    setKpiTitle(cleanValues.kpiTitle);
    setKpiDataType(cleanValues.kpiDataType);
    setKpiDescription(cleanValues.kpiDescription);
    setKpiCalculationExplanation(cleanValues.kpiCalculationExplanation);
    setKpiCalculationDataPoints(cleanValues.kpiCalculationDataPoints);
    setKpiCalculationCategory(cleanValues.kpiCalculationCategory);
    setKpiDataGranularity(cleanValues.kpiDataGranularity);
    setKpiDataSource(cleanValues.kpiDataSource);
    setKpiFunctionalOwner(cleanValues.kpiFunctionalOwner);
    setKpiTargets(cleanValues.kpiTargets);
    setKpiSource(cleanValues.kpiSource);
    setKpiNotes(cleanValues.kpiNotes);
    setKpiValidFrom(cleanValues.kpiValidFrom);
    setKpiValidTo(cleanValues.kpiValidTo);
    setKpiDashboardNotes(cleanValues.kpiDashboardNotes);
    setKpiTranslationRule(cleanValues.kpiTranslationRule);
    setErrors({});
  };
  const [kpiDefinitionRecord, setKpiDefinitionRecord] = React.useState(
    kpiDefinitionModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getKpiDefinition.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getKpiDefinition
        : kpiDefinitionModelProp;
      setKpiDefinitionRecord(record);
    };
    queryData();
  }, [idProp, kpiDefinitionModelProp]);
  React.useEffect(resetStateValues, [kpiDefinitionRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          kpiCode,
          kpiArea: kpiArea ?? null,
          kpiTitle,
          kpiDataType: kpiDataType ?? null,
          kpiDescription,
          kpiCalculationExplanation,
          kpiCalculationDataPoints,
          kpiCalculationCategory: kpiCalculationCategory ?? null,
          kpiDataGranularity: kpiDataGranularity ?? null,
          kpiDataSource: kpiDataSource ?? null,
          kpiFunctionalOwner: kpiFunctionalOwner ?? null,
          kpiTargets: kpiTargets ?? null,
          kpiSource: kpiSource ?? null,
          kpiNotes: kpiNotes ?? null,
          kpiValidFrom: kpiValidFrom ?? null,
          kpiValidTo: kpiValidTo ?? null,
          kpiDashboardNotes: kpiDashboardNotes ?? null,
          kpiTranslationRule: kpiTranslationRule ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateKpiDefinition.replaceAll("__typename", ""),
            variables: {
              input: {
                id: kpiDefinitionRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "KpiDefinitionUpdateForm")}
      {...rest}
    >
      <TextField
        label="Kpi code"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || kpiDefinitionModelProp)}
